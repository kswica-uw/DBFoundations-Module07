...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_KSwica')
	 Begin 
	  Alter Database [Assignment07DB_KSwica] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_KSwica;
	 End
	Create Database Assignment07DB_KSwica;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_KSwica;
...
CREATE FUNCTION dbo.fMonthYear (@Date Date)
 RETURNS NVARCHAR(100) 
 AS
  BEGIN
   RETURN( SELECT DateName(MONTH,@Date) + ', ' + + str(DatePart(yyyy, @Date), 4))
 END; 
GO 

SELECT 
	ProductName,
	[InventoryDate]= dbo.fMonthYear(InventoryDate), 
	[Count]
	FROM vProducts as p 
		JOIN vInventories as i 
			ON p.ProductID=i.ProductID
...
SELECT
	ProductName,
	InventoryDate,
	[DateConverted]=CAST(InventoryDate as Date),
	[Count]
